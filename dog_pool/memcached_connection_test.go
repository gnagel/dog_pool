...
func MemcachedConnectionSpecs(c gospec.Context) {
	...
	c.Specify("[MemcachedConnection] Opening connection to Valid Host/Port has no errors", func() {
		connection := MemcachedConnection{Url: "127.0.0.1:6992", Logger: &memcached_connection_logger}
		defer connection.Close()

		// Start the server ...
		cmd := exec.Command("memcached-server", "--port", "6992")
		err := cmd.Start()
	...
	c.Specify("[MemcachedConnection] Ping (-->Cmd-->Append+GetReply) (re-)opens the connection automatically", func() {
		connection := MemcachedConnection{Url: "127.0.0.1:6993", Logger: &memcached_connection_logger}
		defer connection.Close()

		// Start the server ...
		cmd := exec.Command("memcached-server", "--port", "6993")
		err := cmd.Start()
		c.Expect(err, gospec.Equals, nil)
		if err != nil {
			// Abort on errors
	...
	c.Specify("[MemcachedConnection] Ping to invalid Host/Port has errors", func() {
		connection := MemcachedConnection{Url: "127.0.0.1:6994", Logger: &memcached_connection_logger}
		defer connection.Close()

		// Start the server ...
		cmd := exec.Command("memcached-server", "--port", "6994")
		err := cmd.Start()
		c.Expect(err, gospec.Equals, nil)
		if err != nil {
			// Abort on errors
